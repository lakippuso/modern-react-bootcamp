...
class FullHouse extends Rule {
  evalRoll = dice => {
    // do any of the counts has 3 and 2?
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    ...
    var seqError = true;
    var correctSeq = Array.from({length : 4}, (n, idx) => n  = d[0] + idx);
    for(var i =0; i < correctSeq.length; i++){
      if(correctSeq[i] !== d[i]){
        seqError = false;
        break;
      }
    }
    // small straight must be 4 sequancial dice

    return d.length >= 4 && seqError ? this.score : 0;
  };
}
...
class LargeStraight extends Rule {
  evalRoll = dice => {
    ...
    // small straight must be 5 different dice & only one can be a 1 or a 6
